...
def update_payment(site, transaction_id, transaction_detail):
    ...
    submitted_datetime = datetime.strptime(
        transaction_detail.submitTimeUTC.pyval, "%Y-%m-%dT%H:%M:%S.%f%z"
    )
    ...
    try:
        payment = Payment.objects.get(
            ...
            status__lt=PurchaseStatus.VOID,
        )
...
def subscription_save_transaction(site, transaction_id, transaction_detail):
    ...
    try:
        ...
        logger.info(
            f"AuthorizeCaptureAPI renew_subscription_task subscription {subscription.pk} renewed"
        )
        ...
        logger.info(
            f"AuthorizeCaptureAPI ObjectDoesNotExist Payment for subscription: {subscription_id} - exce: {exce}"
        )
        logger.info(
            f"AuthorizeCaptureAPI renew_subscription_task subscription {subscription.pk} renewed"
        )
    ...
    payment.save()
    logger.info(
        f"AuthorizeCaptureAPI subscription_save_transaction: payment {payment.pk} updated"
    )
    ...
    if payment_status == PurchaseStatus.SETTLED:
        ...
        logger.info(
            f"AuthorizeCaptureAPI subscription_save_transaction: subscription renewed {subscription.pk}"
        )
...
class AuthorizeNetBaseAPI(View):
    ...
    def is_valid_post(self, site):
        logger.info(
            f"AuthorizeNetBaseAPI is_valid_post: Request body: {self.request.body}"
        )
        logger.info(
            f"AuthorizeNetBaseAPI is_valid_post: X ANET SIGNATURE: {self.request.META.get('HTTP_X_ANET_SIGNATURE')}"
        )

        if "HTTP_X_ANET_SIGNATURE" not in self.request.META:
            logger.warning(
                "AuthorizeNetBaseAPI is_valid_post: Signature Key not it request"
            )
            return False
        ...
        try:
            ...
            if credentials.instance and credentials.instance.private_key:
                hash_value = hmac.new(
                    bytes(credentials.instance.private_key, "utf-8"),
                    self.request.body,
                    hashlib.sha512,
                ).hexdigest()
            elif settings.AUTHORIZE_NET_SIGNATURE_KEY:
                hash_value = hmac.new(
                    bytes(settings.AUTHORIZE_NET_SIGNATURE_KEY, "utf-8"),
                    self.request.body,
                    hashlib.sha512,
                ).hexdigest()
            else:
            ...
            logger.error(
                f"ERROR AuthorizeNetBaseAPI is_valid_post: TypeError Exception: {exce}"
            )
...
class AuthorizeCaptureAPI(AuthorizeNetBaseAPI):
    ...
    def is_request_valid(self, site, request_data):
        ...
        if not request_data.get("payload", {}).get("id"):
            logger.error(
                f"ERROR AuthorizeCaptureAPI post: No transaction id request data: {request_data}"
            )
            return False

        if not self.is_valid_post(site):
            logger.error(
                f"ERROR AuthorizeCaptureAPI post: Request was denied: {self.request}"
            )
            return False
    ...
    def post(self, *args, **kwargs):
        try:
            ...
            if not self.is_request_valid(site, request_data):
                ...
                return HttpResponse(
                    status=200, content="AuthorizeCaptureAPI post: Request is invalid"
                )

            transaction_id = request_data.get("payload").get("id")
            logger.info(
                f"AuthorizeCaptureAPI post: Getting transaction detail for id: {transaction_id}"
            )
            ...
            if not hasattr(transaction_detail, "subscription"):
                logger.info(
                    f"AuthorizeCaptureAPI post: updating payment for transaction: {transaction_id}"
                )
                update_payment(site, transaction_id, transaction_detail)

            elif transaction_detail:
                logger.info(
                    f"AuthorizeCaptureAPI post: saving subscription transaction: {transaction_id}"
                )
                subscription_save_transaction(site, transaction_id, transaction_detail)
            else:
                logger.error(
                    f"ERROR AuthorizeCaptureAPI post: No transaction detail for transaction: {transaction_id}"
                )
        ...
        return HttpResponse(
            status=200, content="AuthorizeCaptureAPI post event finished"
        )
...
class VoidAPI(AuthorizeNetBaseAPI):
    ...
    def post(self, *args, **kwargs):
        ...
        if request_data.get("eventType") != "net.authorize.payment.void.created":
            logger.error(
                f"ERROR VoidAPI post: wrong event type: {request_data.get('eventType')}"
            )
            return JsonResponse({"msg": "Event type is incorrect"})

        Payment.objects.filter(
            profile__site=site, transaction=request_data.get("payload").get("id")
        ).update(status=PurchaseStatus.VOID)
        logger.info(
            f"VoidAPI post: payment with transaction voided: {request_data.get('payload').get('id')}"
        )
...
class GetSettledTransactionsView(FormMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            settle_authorizenet_transactions(
                site, form.cleaned_data["start_date"], form.cleaned_data["end_date"]
            )

        return HttpResponseRedirect(
            self.request.META.get("HTTP_REFERER", self.get_success_url())
        )
