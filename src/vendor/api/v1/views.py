...
from vendor.models import CustomerProfile, Offer, Payment, Receipt, Subscription
...
class AddToCartView(View):
    ...
    def add_offer_to_customer_profile_cart(self, customer_profile, offer):
        ...
        if customer_profile.has_product(all_products) and not offer.allow_multiple:
            ...
            cart.order_items.filter(offer__products__in=all_products).exists()
...
class AddOfferToProfileView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        customer_profile = get_object_or_404(
            CustomerProfile, uuid=kwargs.get("uuid_profile")
        )
        ...
        if offer.current_price() or cart.total:
            ...
            return redirect(
                reverse(
                    "vendor_admin:manager-profile",
                    kwargs={"uuid": customer_profile.uuid},
                )
            )
        ...
        return redirect(
            reverse(
                "vendor_admin:manager-profile", kwargs={"uuid": customer_profile.uuid}
            )
        )
...
class SubscriptionPriceUpdateView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        subscription = get_object_or_404(
            Subscription,
            profile__site=site,
            uuid=self.request.POST.get("subscription_uuid"),
        )
        offer = get_object_or_404(
            Offer, site=site, uuid=self.request.POST.get("offer_uuid")
        )

        processor = get_site_payment_processor(site)(site)
        processor.subscription_update_price(
            subscription, offer.current_price(), request.user
        )
