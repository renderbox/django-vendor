...
from vendor.config import (
    DEFAULT_CURRENCY,
    STRIPE_BASE_COMMISSION,
    STRIPE_RECURRING_COMMISSION,
...
class StripeQueryBuilder:
    ...
    def make_clause_template(
        self, field=None, operator=None, key=None, value=None, next_operator=None
    ):
        return {
            "field": field,
            "operator": operator,
    ...
    def build_search_query(self, stripe_object_class, search_clauses):
        ...
        if not isinstance(search_clauses, list):
            logger.error(
                f"Passed in params {search_clauses} is not a list of search clauses (dicts)"
            )
            return query
        ...
        for query_obj in search_clauses:
            ...
            if not self.search_clause_checks_pass(query_obj):
                logger.error(
                    f"StripeQueryBuilder.build_search_query: search clause {query_obj} is not valid"
                )
                return query
            ...
            if self.is_valid_field(stripe_object_class, field):
                ...
                logger.error(
                    f"StripeQueryBuilder.build_search_query: {field} is not valid for {stripe_object_class}"
                )
                return query
...
class StripeProcessor(PaymentProcessorBase):
    ...
    def processor_setup(self, site, source=None):
        ...
        if self.credentials.instance:
            ...
            raise ValueError(
                "StripeProcessor missing keys in settings: STRIPE_PUBLIC_KEY"
            )
    ...
    def validate_invoice_customer_in_stripe(self):
        ...
        stripe_customer = self.stripe_get_object(
            self.stripe.Customer, self.invoice.profile.meta.get("stripe_id")
        )
    ...
    def get_stripe_webhook_endpoints(self):
        ...
        starting_after = None
        while has_more:
            stripe_endpoint_object = self.stripe.WebhookEndpoint.list(
            ...
            if endpoints := stripe_endpoint_object.get("data", []):
                ...
                if hasattr(stripe_endpoint_object.get("data", [])[-1:][0], "id"):
                    starting_after = stripe_endpoint_object.get("data", [])[-1:][0][
                        "id"
                    ]
    ...
    def check_stripe_has_invoice_payment_succeeded_endpoint(self):
        ...
        platform_base_domains = [
            base.lstrip(".") for base in settings.ALLOWED_HOSTS if base.startswith(".")
        ]
        ...
        if endpoint_to_check not in endpoints:
            response = self.stripe.WebhookEndpoint.create(
                enabled_events=["invoice.payment_succeeded"], url=endpoint_to_check
            )

            if response.secret is None:
                logger.error(
                    f"Stripe endpoint {endpoint_to_check} was not created {response}"
                )
    ...
    def stripe_query_object(self, stripe_object_class, query, limit=100, page=None):
        if isinstance(query, dict):
            ...
            query_data = {"query": query, "limit": limit, "page": page}
    ...
    def stripe_get_object(self, stripe_object_class, object_id, expand=None):
        if expand:
            ...
            object_id = {"id": object_id, "expand": expand}
    ...
    def stripe_list_objects(self, stripe_object_class, limit=100, starting_after=None):
        object_data = {"limit": limit, "starting_after": starting_after}
    ...
    def get_all_stripe_list_objects(self, stripe_object):
        ...
        starting_after = None
        while True:
            objs = self.stripe_list_objects(
                stripe_object, limit=100, starting_after=starting_after
            )
    ...
    def build_price(self, price, currency):
        if (
            "stripe" not in price.offer.meta
        ...
        price_data = {
            ...
            "metadata": {"site": price.offer.site.domain, "pk": price.pk},
        }
        ...
        if price.offer.terms < TermType.PERPETUAL:
            price_data["recurring"] = {
                "interval": (
                    "month"
                    if price.offer.term_details["term_units"] == TermDetailUnits.MONTH
                    else "year"
                ),
    ...
    def build_coupon(self, offer, currency):
        ...
        if offer.terms < TermType.PERPETUAL:
            coupon_data["amount_off"] = self.convert_decimal_to_integer(
                offer.get_trial_discount()
            )
            coupon_data["duration"] = (
                "once" if offer.term_details["trial_occurrences"] <= 1 else "repeating"
            )
                ...
                "address": {
                    ...
                    "country": Country.names[
                        Country.values.index(
                            int(self.billing_address.cleaned_data.get("country"))
                        )
                    ],
    ...
    def build_payment_intent(
        self, amount, payment_method_id, currency=DEFAULT_CURRENCY
    ):
    ...
    def build_subscription(self, subscription, payment_method_id):
        ...
        if hasattr(self.invoice, "coupon_code") and (
        ...
        return {
            ...
            "items": [{"price": stripe_meta["prices"][str(price.pk)]}],
    ...
    def build_invoice_line_item(self, order_item, invoice_id):
        ...
        if (
            order_item.offer.has_trial()
            or order_item.offer.has_valid_billing_start_date()
            or order_item.offer.discount()
        ):
            line_item["discounts"] = [
                {"coupon": order_item.offer.meta["stripe"].get("coupon_id")}
            ]
    ...
    def build_refund(self, refund_form):
        int_amount = self.convert_decimal_to_integer(
            refund_form.cleaned_data["refund_amount"]
        )
        return {
    ...
    def get_stripe_customers(self, site, expand=None):
        ...
        customer_search = self.get_all_stripe_search_objects(
            self.stripe.Customer, {"query": query}
        )
    ...
    def get_customer_profile(self, stripe_customer, site=None):
        ...
        try:
            customer_profile = CustomerProfile.objects.get(
                site=site, user__email=stripe_customer.email
            )
        except ObjectDoesNotExist:
            logger.error(
                f"get_customer_profile CustomerProfile Not Found email:{stripe_customer.email} site: {site.domain}"
            )
            ...
            logger.error(
                f"get_customer_profile Exception: {exce} for email:{stripe_customer.email} site: {site.domain} "
            )
    ...
    def get_customer_profile_and_stripe_customer(self, stripe_customer_id):
        ...
        if not customer_profile:
            logger.info(
                f"Customer Profile not found for stripe_customer {stripe_customer.id} site: {self.site}"
            )
            return customer_profile, stripe_customer
    ...
    def get_vendor_customers_in_stripe(self, customer_email_list, site):
        ...
        users = CustomerProfile.objects.filter(
            user__email__iregex=r"("
            + "|".join(customer_email_list)
    ...
    def get_vendor_customers_not_in_stripe(self, customer_email_list, site):
        ...
        """
        users = (
            CustomerProfile.objects.filter(site=site)
            .exclude(
                user__email__iregex=r"("
                + "|".join(customer_email_list)
                + ")",  # iregex used for case insensitive list match
    ...
    def create_stripe_customers(self, customers):
        for profile in customers:
            ...
            new_stripe_customer = self.stripe_create_object(
                self.stripe.Customer, profile_data
            )
    ...
    def update_stripe_customers(self, customers):
        for profile in customers:
            ...
            profile_data = self.build_customer(profile)
            existing_stripe_customer = self.stripe_update_object(
                self.stripe.Customer, customer_id, profile_data
            )
    ...
    def get_customer_payment_methods(self, customer_id):
        payment_methods = self.stripe_call(
            self.stripe.Customer.list_payment_methods, customer_id
        )

        if not payment_methods or not getattr(payment_methods, "data", None):
            # offline/test fallback
            return [SimpleNamespace(id="stub_payment_method")]
            ...
            if "card" in source_id:
                ...
                if stripe_card:
                    if stripe_card["exp_year"] and stripe_card["exp_month"]:
                        exp_date = (
                            f"{stripe_card['exp_year']}-{stripe_card['exp_month']}"
                        )
    ...
    def get_stripe_customer_from_email(self, email):
        ...
        email_clause = query_builder.make_clause_template(
            field="email", value=email, operator=query_builder.EXACT_MATCH
        )
        ...
        if len(query_result["data"]) == 1:
            ...
            logger.info(
                f"Multiple stripe customer found for email: {email} on site: {self.site}"
            )
        else:
            logger.info(
                f"No Stripe Customer Found for email: {email} on site: {self.site}"
            )
    ...
    def get_stripe_product(self, site, pk):
        ...
        query = self.query_builder.build_search_query(
            self.stripe.Product, [pk_clause, site_clause]
        )
    ...
    def does_product_exist(self, name, metadata):
        ...
        query = self.query_builder.build_search_query(
            self.stripe.Product, [name_clause, metadata_clause]
        )
    ...
    def get_site_stripe_products(self, site):
        ...
        product_search = self.get_all_stripe_search_objects(
            self.stripe.Product, {"query": query}
        )

        return product_search

    def get_vendor_offers_in_stripe(self, offer_pk_list, site):
        offers = Offer.objects.filter(site=site, pk__in=offer_pk_list).exclude(
            is_promotional=True
        )
        return offers
    ...
    def get_vendor_offers_not_in_stripe(self, offer_pk_list, site):
        offers = (
            Offer.objects.filter(site=site)
            .exclude(pk__in=offer_pk_list)
            .exclude(is_promotional=True)
        )
        return offers
    ...
    def create_stripe_product(self, offer):
        ...
        offer.save()
        logger.info(
            f"create_stripe_product Stripe Product created: {stripe_product.id} from offer {offer} site {offer.site}"
        )
    ...
    def sync_offer(self, offer):
        ...
        stripe_product = self.stripe_update_object(
            self.stripe.Product, product_id, product_data
        )
    ...
    def does_stripe_and_vendor_price_match(self, stripe_price, vendor_price):
        ...
        if "recurring" in vendor_price:
            ...
            if (
                not stripe_price.recurring["interval"]
                == vendor_price["recurring"]["interval"]
            ):
                return False

            if (
                not stripe_price.recurring["interval_count"]
                == vendor_price["recurring"]["interval_count"]
            ):
                return False
    ...
    def get_stripe_prices_for_product(self, product):
        product_clause = self.query_builder.make_clause_template(
            field="product", value=product, operator=self.query_builder.EXACT_MATCH
        )

        query = self.query_builder.build_search_query(
            self.stripe.Price, [product_clause]
        )

        search_data = self.get_all_stripe_search_objects(
            self.stripe.Price, {"query": query}
        )
    ...
    def does_price_exist(self, product, metadata):
        ...
        query = self.query_builder.build_search_query(
            self.stripe.Price, [product_clause, metadata_clause]
        )
    ...
    def create_price_with_product(self, product):
        price_data = {"currency": self.invoice.currency, "product": product}
    ...
    def create_stripe_product_prices(self, offer):
        for price in offer.prices.all():
            ...
            if not stripe_price:
                logger.error(
                    f"create_stripe_product_prices price not created: {self.transaction_info}"
                )
                return None
    ...
    def sync_offer_prices(self, offer):
        ...
        for stripe_price in stripe_product_prices:
            ...
            logger.info(
                f"sync_offer_prices: Stripe Price Updated: ({stripe_price.id}, {price.pk})"
            )
    ...
    def match_coupons(self, coupons, offer):
        ...
        for coupon in coupons:
            if (
                coupon["metadata"]["site"] == offer.site.domain
                and coupon["amount_off"] == amount_off
    ...
    def get_or_create_invoice_from_stripe_invoice(
        self, stripe_invoice, offer, customer_profile
    ):
        invoice = None
        created = False
        ...
        try:
            invoice, created = Invoice.objects.get_or_create(
                ...
                defaults={
                    ...
                    "ordered_date": timezone.datetime.fromtimestamp(
        ...
        if created:
            ...
            logger.info(
                f"get_or_create_invoice_from_stripe_invoice Invoice Created: ({invoice.pk}, {stripe_invoice.id})"
            )
    ...
    def get_offers_from_invoice_line_items(self, stripe_line_items):
        ...
        for line_item in stripe_line_items:
            stripe_product = self.stripe_get_object(
                self.stripe.Product, line_item.product
            )
    ...
    ##########
    def get_or_create_payment_from_stripe_payment_and_charge(
        self, invoice, stripe_payment_method, stripe_charge
    ):
        payment = None
        created = False
        ...
        try:
            payment, created = Payment.objects.get_or_create(
                ...
                defaults={
                    ...
                    "amount": self.convert_integer_to_decimal(
                        stripe_charge.amount_captured
                    ),
                    "invoice": invoice,
                    "submitted_date": timezone.datetime.fromtimestamp(
                        stripe_charge.created, tz=timezone.utc
                    ),
                    "transaction": stripe_charge.id,
                    ...
                    "result": {
                        "account_number": (
                            ...
                            if "card" in stripe_payment_method
                            else ""
                        ),
    ...
    def get_or_create_subscription_receipt_from_stripe_charge(
        self, invoice, payment, stripe_charge
    ):
        receipt = None
        created = False
        try:
            receipt, created = Receipt.objects.get_or_create(
                ...
                defaults={
                    ...
                    "end_date": invoice.order_items.first().offer.get_offer_end_date(
                        start_date=timezone.datetime.fromtimestamp(
                            stripe_charge.created, tz=timezone.utc
                        )
                    ),
            ...
            dup_receipts = Receipt.objects.filter(
                profile=invoice.profile, transaction=stripe_charge.id
            )
        ...
        if created:
            ...
            logger.info(
                f"get_or_create_receipt_from_stripe_charge Receipt created: ({receipt.pk}, {stripe_charge.id})"
            )
            ...
            try:
                receipt, created = Receipt.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "end_date": order_item.offer.get_offer_end_date(
                            start_date=timezone.datetime.fromtimestamp(
                                stripe_charge.created, tz=timezone.utc
                            )
                        ),
    ...
    def get_stripe_subscriptions(self, site):
        ...
        query = self.query_builder.build_search_query(
            self.stripe.Subscription, [clause]
        )

        subscription_search = self.get_all_stripe_search_objects(
            self.stripe.Subscription, {"query": query}
        )

        return subscription_search

    def get_or_create_subscription_from_stripe_subscription(
        self, customer_profile, stripe_subscription
    ):
        created = False
        subscription = None
        try:
            subscription, created = Subscription.objects.get_or_create(
                profile=customer_profile, gateway_id=stripe_subscription.id
            )
        except MultipleObjectsReturned:
            logger.error(
        ...
        if subscription:
            subscription.status = self.get_subscription_status(
    ...
    def get_subscription(self, stripe_subscription):
        ...
        try:
            ...
            logger.error(
                f"Subscription does not exists for Stripe Subscription: {stripe_subscription.id}"
            )
            return None
        except MultipleObjectsReturned:
            logger.error(
                f"Multiple Subscriptions found for Stripe Subscription: {stripe_subscription.id}"
            )
        except Exception as exce:
            logger.error(
                f"Get Subscription Exception: {exce} for Stripe Subscription {stripe_subscription.id}"
            )
    ...
    def create_subscription(self, payment_method_id, subscription_extras):
        ...
        stripe_setup_intent = self.stripe_create_object(
            self.stripe.SetupIntent, setup_intent_object
        )
        if not stripe_setup_intent:
            raise Exception(
                f"Could not create stripe_setup_intent transaction_info: {self.transaction_info}"
            )

        subscription_obj = self.build_subscription(
            self.invoice.order_items.first(), payment_method_id
        )
        subscription_obj |= subscription_extras

        stripe_subscription = self.stripe_create_object(
            self.stripe.Subscription, subscription_obj
        )
        if not stripe_subscription or stripe_subscription.status == "incomplete":
            self.transaction_succeeded = False
            raise Exception(
                f"Subscription Failed to be created: {self.transaction_info}"
            )
    ...
    def transfer_existing_customer_subscription_to_stripe(self, customer_profile):
        ...
        for subscription in customer_profile.subscriptions.filter(
            status=SubscriptionStatus.ACTIVE
        ):
            ...
            stripe_subscription = self.stripe_get_object(
                self.stripe.Subscription, subscription.gateway_id
            )
            ...
            if offer and not stripe_subscription:
                ...
                try:
                    ...
                    if not stripe_payment_method:
                        raise Exception(
                            f"Could not create stripe_payment_method transaction_info: {self.transaction_info}"
                        )

                    subscription_extras = {
                        "billing_cycle_anchor": offer.get_offer_end_date(
                            last_start_date
                        ),
                    ...
                    transfer_result_msg["success"].append(
                        f"Successfuly Transfered Subscription: {subscription.gateway_id}"
                    )
                    self.update_invoice_status(InvoiceStatus.COMPLETE)

                except Exception as exce:
                    logger.error(
                        f"Creating Subscription Failed, transaction_info: {self.transaction_info}, exception: {exce}"
                    )
                    transfer_result_msg["failed"].append(
                        f"Failed Transfer Subscription {subscription.gateway_id}, exception: {exce}"
                    )
    ...
    def initialize_products(self, site):
        ...
        for subscription in self.invoice.get_recurring_order_items():
            ...
            interval = (
                "month"
                if product_details["term_units"] == TermDetailUnits.MONTH
                else "year"
            )
            metadata = {"key": "site", "value": "site4", "field": "metadata"}
            product_response = self.does_product_exist(
                product_name_full, metadata=metadata
            )
            if product_response:
                ...
                product = self.stripe_call(
                    self.stripe.Product.create,
                    {
                        "name": product_name,
    ...
    def sync_customers(self, site):
        ...
        stripe_customers_emails = [
            customer_obj["email"] for customer_obj in stripe_customers
        ]

        vendor_customers_in_stripe = self.get_vendor_customers_in_stripe(
            stripe_customers_emails, site
        )

        vendor_customers_with_stripe_meta = vendor_customers_in_stripe.filter(
            meta__has_key="stripe_id"
        )
        vendor_customers_without_stripe_meta = vendor_customers_in_stripe.exclude(
            meta__has_key="stripe_id"
        )

        vendor_customers_not_in_stripe = self.get_vendor_customers_not_in_stripe(
            stripe_customers_emails, site
        )
        vendor_customer_to_create = (
            vendor_customers_not_in_stripe | vendor_customers_without_stripe_meta
        )
    ...
    def sync_offers(self, site):
        ...
        offers_in_vendor_with_stripe_meta = offers_in_vendor.filter(
            meta__has_key="stripe"
        )
        offers_in_vendor_without_stripe_meta = offers_in_vendor.exclude(
            meta__has_key="stripe"
        )
        ...
        for update_offer_meta in offers_in_vendor_without_stripe_meta:
            for stripe_product in stripe_products:
                if stripe_product["metadata"]["pk"] == update_offer_meta.pk:
                    update_offer_meta.meta.update(
                        {"stripe": {"product_id": stripe_product.id}}
                    )
                    update_offer_meta.save()

        self.create_offers(offers_to_create)
        self.update_offers(
            offers_in_vendor_with_stripe_meta | offers_in_vendor_without_stripe_meta
        )
        logger.info("StripeProcessor sync_offers Finished")

    def sync_stripe_subscription(self, site, stripe_subscription):
        stripe_customer = self.stripe_get_object(
            self.stripe.Customer, stripe_subscription.customer
        )
        customer_profile = self.get_customer_profile(stripe_customer, site)

        if customer_profile:
            subscription, created = (
                self.get_or_create_subscription_from_stripe_subscription(
                    customer_profile, stripe_subscription
                )
            )

            stripe_product = self.stripe_get_object(
                self.stripe.Product, stripe_subscription.plan.product
            )
            offer = self.get_offer_from_stripe_product(stripe_product)

            if offer:
                stripe_sub_invoices = self.get_subscription_invoices(
                    stripe_subscription.id
                )

                for stripe_invoice in stripe_sub_invoices:
                    invoice, created = self.get_or_create_invoice_from_stripe_invoice(
                        stripe_invoice, offer, customer_profile
                    )

                    if stripe_invoice.charge:
                        stripe_charge = self.stripe_get_object(
                            self.stripe.Charge, stripe_invoice.charge
                        )
                        stripe_payment_method = self.stripe_get_object(
                            self.stripe.PaymentMethod, stripe_charge.payment_method
                        )

                        payment, created = (
                            self.get_or_create_payment_from_stripe_payment_and_charge(
                                invoice, stripe_payment_method, stripe_charge
                            )
                        )
                        if created:
                        ...
                        if payment.status == PurchaseStatus.SETTLED:
                            receipt, created = (
                                self.get_or_create_subscription_receipt_from_stripe_charge(
                                    invoice, payment, stripe_charge
                                )
                            )
    ...
    def sync_stripe_subscriptions(self, site):
        ...
        for stripe_subscription in stripe_subscriptions:
            logger.info(
                f"sync_stripe_subscriptions: syncing subscription: {stripe_subscription.id}"
            )
    ...
    def create_setup_intent(self, setup_intent_data):
        setup_intent = self.stripe_create_object(
            self.stripe.SetupIntent, setup_intent_data
        )
    ...
    def create_payment_intent(self, payment_intent_data):
        ...
        payment_intent = self.stripe_create_object(
            self.stripe.PaymentIntent, payment_intent_data
        )
    ...
    def process_payment(self):
        ...
        for order_item in self.invoice.get_one_time_transaction_order_items():
            ...
            if not line_item_data:
                return None
            stripe_line_item = self.stripe_create_object(
                self.stripe.InvoiceItem, line_item_data
            )
        ...
        stripe_invoice = self.stripe_update_object(
    ...
    def subscription_payment(self, subscription):
        ...
        subscription_obj = self.build_subscription(
            subscription, stripe_payment_method.id
        )
    ...
    def charge_customer_profile(self):
        ...
        stripe_customer_payment_methods = self.get_customer_payment_methods(
            self.invoice.profile.meta["stripe_id"]
        )
        ...
        amount = self.convert_decimal_to_integer(
            self.invoice.get_one_time_transaction_total()
        )
        payment_method_id = stripe_customer_payment_methods[0].id
        payment_intent_data = self.build_payment_intent(amount, payment_method_id)
        stripe_payment_intent = self.stripe_create_object(
            self.stripe.PaymentIntent, payment_intent_data
        )
        ...
        self.stripe_call(stripe_invoice.pay, {"payment_method": payment_method_id})

        if self.transaction_succeeded:
            self.transaction_id = stripe_invoice.payment_intent

    def subscription_cancel(self, subscription):
        stripe_subscription = self.stripe_delete_object(
            self.stripe.Subscription, subscription.gateway_id
        )
    ...
    def subscription_update_payment(self, subscription):
        ...
        stripe_subscription_object = self.stripe_get_object(
            self.stripe.Subscription, subscription.gateway_id
        )
        ...
        if current_payment_method:
            payment_method_data["billing_details"]["address"] = (
                current_payment_method.billing_details.get("address")
            )
        ...
        self.stripe.Subscription.modify(
            subscription.gateway_id, default_payment_method=stripe_payment_method
        )
