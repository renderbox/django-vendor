...
def get_available_country_choices():
    if isinstance(COUNTRY_CHOICE, list):
        return [
            ...
            if country.name in COUNTRY_CHOICE
        ]
    return Country.choices


class PriceForm(forms.ModelForm):
    CHOICES = [("not_free", _("Purchase Price")), ("free", _("Free"))]
    price_select = forms.ChoiceField(
        label="", choices=CHOICES, widget=forms.widgets.RadioSelect()
    )
...
class OfferForm(forms.ModelForm):
    products = forms.ModelMultipleChoiceField(
        label=_("Available Products:"),
        required=True,
        queryset=Product.objects.filter(site=settings.SITE_ID, available=True),
    )
    ...
    def clean(self):
        ...
        term_detail_defaults = [
            False
            for default in list(offer_term_details_default().keys())
            if default not in cleaned_data["term_details"]
        ]

        if (
            self.data["terms"] == str(TermType.SUBSCRIPTION)
            and False in term_detail_defaults
        ):
...
class AddressForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["address_2"].widget.attrs.update(
            {"placeholder": _("Enter Apt, Suite, Unit, Building, Floor, etc.")}
        )
...
class AccountInformationForm(AddressForm):
    ...
    select_state = forms.ChoiceField(
        label=_("State"), choices=USAStateChoices.choices, required=False
    )
    prefix = "account_info"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["select_state"].choices = [("", _("Select State"))] + self.fields[
            "select_state"
        ].choices
    ...
    def __init__(self, placeholder=None, *args, **kwargs):
        super(CreditCardField, self).__init__(
            widget=forms.widgets.TextInput(
                attrs={"placeholder": placeholder, "type": "tel"}
            ),
            *args,
            **kwargs,
        )
...
class CreditCardForm(PaymentFrom):
    ...
    expire_month = forms.ChoiceField(
        ...
        choices=[(x, f"{x:02d}") for x in range(1, 13)],
    )
    expire_year = forms.ChoiceField(required=True, label=_("Expiration Year"))
    cvv_number = forms.CharField(
        required=True,
        label=_("CVV Number"),
        max_length=4,
        min_length=3,
        widget=forms.TextInput(attrs={"size": "4", "placeholder": _("Enter CVV")}),
    )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        today = datetime.now()
        self.fields["expire_year"].choices = [
            (x, x) for x in range(today.year, today.year + 15)
        ]
        self.fields["expire_year"].initial = (today.year, today.year)
        self.fields["full_name"].widget.attrs.update(
            {"placeholder": _("Enter Name on Card")}
        )
    ...
    def clean(self):
        ...
        if not self.expiration_date_valid(expire_month, expire_year):
            cleaned_data.pop("expire_month", None)
            cleaned_data.pop("expire_year", None)
    ...
    def clean_expire_month(self):
        ...
        if not expire_month:
            raise forms.ValidationError(
                _("You must select a valid expiration month"), code="required"
            )
    ...
    def clean_expire_year(self):
        ...
        if not expire_year:
            raise forms.ValidationError(
                _("You must select a valid expiration year"), code="required"
            )
    ...
    def expiration_date_valid(self, expire_month, expire_year):
        ...
        if datetime.now() > expire:
            self._errors["expire_year"] = self.error_class(
...
class DateRangeForm(forms.Form):
    start_date = forms.DateField(
        required=False, label=_("Start Date"), widget=SelectDateWidget()
    )
    end_date = forms.DateField(
        required=False, label=_("End Date"), widget=SelectDateWidget()
    )
...
class SubscriptionForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not ("site" in self.initial or "site" in self.data):
            raise KeyError(
                "site needs to be inluded in either initial or data attributes"
            )
...
class StartDateForm(forms.Form):
    start_date = forms.DateTimeField(
        widget=forms.SelectDateWidget(attrs={"class": "datetime"})
    )
