...
class ModelInvoiceTests(TestCase):
    ...
    def test_add_quantity(self):
        ...
        start_quantity = (
            self.existing_invoice.order_items.filter(offer=self.shirt_offer)
            .first()
            .quantity
        )
    ...
    def test_remove_quantity_zero(self):
        start_quantity = (
            self.existing_invoice.order_items.filter(offer=self.hamster)
            .first()
            .quantity
        )
    ...
    def test_swap_offers_success(self):
        ...
        self.assertTrue(
            self.new_invoice.order_items.filter(offer=free_hulk_offer).exists()
        )
    ...
    def test_save_discounts_vendor_notes(self):
        free_month_offer = Offer.objects.get(pk=7)
        discount = (
            free_month_offer.current_price() - free_month_offer.get_trial_amount()
        )
        self.new_invoice.add_offer(free_month_offer)
    ...
    def test_order_item_price_msrp(self):
        ...
        self.new_invoice.add_offer(wheel_offer)
        self.assertEqual(
            self.new_invoice.order_items.first().price, wheel_offer.get_msrp()
        )
        self.assertNotEqual(
            self.new_invoice.order_items.first().price, wheel_offer.current_price()
        )
    ...
    def test_order_item_price_current_price(self):
        ...
        self.assertNotEqual(
            self.new_invoice.order_items.first().price, free_month.get_msrp()
        )
        self.assertEqual(
            self.new_invoice.order_items.first().price, free_month.current_price()
        )
    ...
    def test_order_item_with_trial_amount(self):
        ...
        self.new_invoice.add_offer(free_month)
        self.assertEqual(
            self.new_invoice.order_items.first().trial_amount,
    ...
    def test_order_item_no_trial_amount(self):
        ...
        self.assertEqual(
            self.new_invoice.get_recurring_total(), month_offer.current_price()
        )
        self.assertEqual(self.new_invoice.get_discounts(), 0)
        self.assertEqual(
            self.new_invoice.order_items.first().trial_amount,
    ...
    def test_invoice_global_discount(self):
        ...
        self.new_invoice.save()
        self.assertEqual(
            self.new_invoice.total,
            month_offer.current_price() - self.new_invoice.global_discount,
        )
    ...
    def test_soft_delete(self):
        ...
        deleted_invoice_difference = (
            Invoice.objects.all().count() - Invoice.not_deleted.count()
        )
    ...
    def test_clear_promos_when_last_item_is_removed(self):
        ...
        not_promotional_order_item_offers_in_invoice = (
            self.existing_invoice.order_items.filter(offer__is_promotional=False)
        )
...
class CartViewTests(TestCase):
    ...
    def test_view_cart_content_loads(self):
        ...
        self.assertContains(
            response, f"<span>${self.invoice.calculate_subtotal():.2f}</span>"
        )
        ...
        self.assertContains(
            response,
            f'<span class="font-weight-bolder">Total ({self.invoice.get_currency_display()}) </span>',
        )
...
class AccountInformationViewTests(TestCase):
    ...
    def test_view_redirect_login(self):
        ...
        self.assertRedirects(
            response, reverse("account_login") + "?next=" + self.view_url
        )
...
class PaymentSummaryViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.view_url = reverse(
            "vendor:purchase-summary", kwargs={"uuid": self.invoice.uuid}
        )
