...
class SubscriptionViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.subscription_detail_uri = reverse('vendor_admin:manager-subscription', kwargs={'uuid': self.subscription.uuid})
        self.subscription_create_uri = reverse('vendor_admin:manager-subscription-create')
        self.subscription_add_payment_uri = reverse('vendor_admin:manager-subscription-add-payment', kwargs={'uuid_subscription': self.subscription.uuid, 'uuid_profile': self.profile.uuid})
    ...
    def test_admin_subscription_detail_fail(self):
        non_exiting_subscriptin_uri = reverse('vendor_admin:manager-subscription', kwargs={'uuid': self.profile.uuid})
    ...
    def test_admin_subscription_add_payment_post_success(self):
        ...
        response = self.client.post(f"{self.subscription_add_payment_uri}?site=1", data=post_data)
