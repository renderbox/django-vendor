...
from vendor.models import CustomerProfile, Invoice, Offer, Price, Receipt
...
class ModelCustomerProfileTests(TestCase):
    ...
    def test_get_cart_items_count_new_user(self):
        ...
        count = self.user.customer_profile.get(
    ...
    def test_gets_checkout_cart(self):
        self.customer_profile_existing.invoices.add(
            Invoice.objects.create(
                status=InvoiceStatus.CHECKOUT, profile=self.customer_profile_existing
            )
        )
    ...
    def test_get_cart_items_count(self):
        ...
        self.assertEqual(
            invoice.order_items.count(),
            self.customer_profile_existing.get_cart_items_count(),
        )
    ...
    def test_get_recurring_receipts(self):
        ...
        self.assertEqual(
            0, len(self.customer_profile.get_one_time_transaction_receipts())
        )
    ...
    def test_get_active_offer_receipts_empty(self):
        offer = Offer.objects.get(pk=3)
        self.assertTrue(
            len(self.customer_profile.get_active_offer_receipts(offer)) == 0
        )
    ...
    def test_get_checkout_invoice_success(self):
        ...
        self.assertEqual(
            1,
            self.customer_profile_existing.invoices.filter(
                status=InvoiceStatus.CART, deleted=False
            ).count(),
        )
...
class AddOfferToProfileView(TestCase):
    ...
    def setUp(self):
        ...
        self.free_offer = Offer.objects.create(
            name="Free Offer",
            start_date=timezone.now(),
    ...
    def test_view_cart_status_code(self):
        url = reverse(
            "vendor_api:manager-profile-add-offer",
            kwargs={
                "uuid_profile": self.customer_profile.uuid,
