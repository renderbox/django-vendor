...
class StripeProcessorTests(TestCase):
    ...
    def setUp(self):
        ...
        self.cus_norrin_radd = {
            "name": "Norrin Radd",
            "email": "norrin@radd.com",
            "metadata": self.valid_metadata,
        }
        self.cus_norrin_radd2 = {
            "name": "Jake Paul",
            "email": "jpaul@radd.com",
            "metadata": self.valid_metadata,
        }
        self.pro_annual_license = {
            "name": "Annual Subscription",
            "metadata": self.valid_metadata,
        }
        self.pro_annual_license2 = {
            "name": "Annual Subscription 2",
            "metadata": self.valid_metadata,
        }
        self.pri_monthly = {
            "currency": "usd",
    ...
    def test_get_stripe_offers(self):
        ...
        if not offers_exist:
            self.processor.stripe_create_object(
    ...
    def test_get_vendor_offers_in_stripe(self):
        ...
        offer1 = Offer.objects.create(
            site=self.site,
            name=self.pro_annual_license["name"],
            start_date=timezone.now(),
        )
        offer2 = Offer.objects.create(
            site=self.site,
            name=self.pro_annual_license2["name"],
            start_date=timezone.now(),
        )
        ...
        offers_exist = [
            name for name in vendor_offer_names if name in stripe_offer_names
        ] or False
        ...
        vendor_offers_in_stripe = self.processor.get_vendor_offers_in_stripe(
            pk_list, self.site
        )
    ...
    def test_get_vendor_offers_not_in_stripe(self):
        ...
        vendor_offers_not_in_stripe = self.processor.get_vendor_offers_not_in_stripe(
            pk_list, self.site
        )
    ...
    def test_get_stripe_customers(self):
        ...
        customers_exists = [
            name for name in test_names if name in customer_names
        ] or False

        if not customers_exists:
            self.stripe_customer1 = self.processor.stripe_create_object(
                self.processor.stripe.Customer, self.cus_norrin_radd
            )
            self.stripe_customer2 = self.processor.stripe_create_object(
                self.processor.stripe.Customer, self.cus_norrin_radd2
            )
    ...
    def test_get_vendor_customers_in_stripe(self):
        ...
        user1 = User.objects.create(
            email=self.cus_norrin_radd["email"],
            first_name=first_name1,
            last_name=last_name1,
            username="test1",
        )
        user2 = User.objects.create(
            email=self.cus_norrin_radd2["email"],
        ...
        customers_in_vendor = self.processor.get_vendor_customers_in_stripe(
            email_list, self.site
        )
        customers_in_vendor_emails = [
            customer.user.email for customer in customers_in_vendor
        ]
    ...
    def test_get_vendor_customers_not_in_stripe(self):
        ...
        customers_not_in_vendor = self.processor.get_vendor_customers_not_in_stripe(
            email_list, self.site
        )
    ...
    def test_create_stripe_customers(self):
        ...
        self.processor.stripe_delete_object(
            self.processor.stripe.Customer, vendor_customer1.meta["stripe_id"]
        )
        self.processor.stripe_delete_object(
            self.processor.stripe.Customer, vendor_customer2.meta["stripe_id"]
        )
    ...
    def test_update_stripe_customers(self):
        stripe_customer1 = self.processor.stripe_create_object(
            self.processor.stripe.Customer, self.cus_norrin_radd
        )
        stripe_customer2 = self.processor.stripe_create_object(
            self.processor.stripe.Customer, self.cus_norrin_radd2
        )

        user1 = User.objects.create(
            email=self.cus_norrin_radd["email"],
            first_name="New First Name1",
            last_name="Last Name1",
            username="test1",
        )
        user2 = User.objects.create(
            email=self.cus_norrin_radd2["email"],
            first_name="New First Name2",
            last_name="Last Name2",
            username="test2",
        )
        vendor_customer1 = CustomerProfile.objects.create(
            site=self.site, user=user1, meta={"stripe_id": stripe_customer1.id}
        )
        vendor_customer2 = CustomerProfile.objects.create(
            site=self.site, user=user2, meta={"stripe_id": stripe_customer2.id}
        )
        ...
        updated_stripe_customer1 = self.processor.stripe_get_object(
            self.processor.stripe.Customer, stripe_customer1.id
        )
        updated_stripe_customer2 = self.processor.stripe_get_object(
    ...
    def test_check_product_does_exist(self):
        ...
        product = self.processor.does_product_exist(
            self.pro_annual_license["name"], metadata=metadata
        )
    ...
    def test_get_product_id_with_name(self):
        ...
        metadata = {"key": "site", "value": self.site.domain, "field": "metadata"}
        product_id = self.processor.get_product_id_with_name(
            self.pro_annual_license["name"], metadata=metadata
        )
    ...
    def test_get_customer_payment_methods(self):
        payment_methods = self.processor.get_customer_payment_methods(
            "cus_Mt98i0KskNWP0X"
        )
...
@skipIf(
    (settings.STRIPE_PUBLIC_KEY or settings.STRIPE_SECRET_KEY) is None,
    "Strip enviornment variables not set, skipping tests",
)
class StripeCRUDObjectTests(TestCase):
    ...
    def test_create_customer_success(self):
        stripe_customer = self.processor.stripe_create_object(
            self.processor.stripe.Customer, self.cus_norrin_radd
        )

        self.processor.stripe_delete_object(
            self.processor.stripe.Customer, stripe_customer.id
        )
    ...
    def test_create_product_success(self):
        ...
        self.processor.stripe_delete_object(
            self.processor.stripe.Product, stripe_product.id
        )
    ...
    def test_get_product_success(self):
        ...
        fetch_product = self.processor.stripe_get_object(
            self.processor.stripe.Product, stripe_product.id
        )
    ...
    def test_update_product_success(self):
        ...
        update_product = self.processor.stripe_update_object(
            self.processor.stripe.Product, stripe_product.id, update_data
        )
    ...
    def test_create_coupon_success(self):
        stripe_coupon = self.processor.stripe_create_object(
            self.processor.stripe.Coupon, self.cou_first_month_free
        )

        stripe_coupon = self.processor.stripe_delete_object(
            self.processor.stripe.Coupon, stripe_coupon.id
        )
    ...
    def test_create_subscription_success(self):
        stripe_cus_norrin_radd = self.processor.stripe_create_object(
            self.processor.stripe.Customer, self.cus_norrin_radd
        )

        stripe_payment_method = self.processor.stripe_create_object(
            self.processor.stripe.PaymentMethod, self.payment_method
        )
        ...
        stripe_pro_monthly = self.processor.stripe_create_object(
            self.processor.stripe.Product, self.pro_monthly_license
        )
        ...
        stripe_subscription = self.processor.stripe_create_object(
            self.processor.stripe.Subscription, subscription_obj
        )
...
class StripeBuildObjectTests(TestCase):
    ...
    def test_build_customer_success(self):
        ...
        stripe_customer = self.processor.stripe_create_object(
            self.processor.stripe.Customer, customer_data
        )

        self.assertIsNotNone(stripe_customer.id)
        self.assertEqual(
            f"{customer_profile.user.first_name} {customer_profile.user.last_name}",
    ...
    def test_build_product_success(self):
        ...
        stripe_product = self.processor.stripe_create_object(
            self.processor.stripe.Product, product_data
        )
